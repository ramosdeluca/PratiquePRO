      ...
      {showCancelConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-fade-in">
          <div className="bg-gray-800 border border-red-500/30 rounded-3xl p-8 max-w-sm w-full shadow-2xl space-y-6 text-center">
            ...
            <div><h3 className="text-xl font-bold mb-2">Cancelar Assinatura?</h3></div>
        ...
        {activeTab === 'practice' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in">
            {AVATARS.map((avatar) => (
              <div 
                key={avatar.name} 
                className={`bg-gray-800 rounded-3xl overflow-hidden border border-gray-700 transition-all group cursor-pointer ${(subscriptionErrorStatus || isCancelled || isPending) ? 'opacity-50 grayscale' : 'hover:border-blue-500/50'}`} 
                onClick={() => handleAvatarClick(avatar)}
              >
                <div className="h-64 overflow-hidden relative">
                    <img src={avatar.avatarImage} className={`w-full h-full object-cover transition-transform duration-500 ${(subscriptionErrorStatus || isCancelled || isPending) ? '' : 'group-hover:scale-110'}`} />
                ...
                <div className="p-6">
                  ...
                  <button className={`mt-4 w-full text-white py-3 rounded-xl text-sm font-bold shadow-lg transition-all ${(subscriptionErrorStatus || isCancelled || isPending) ? 'bg-gray-600 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-500'}`}>
                    {(subscriptionErrorStatus || isCancelled || isPending) ? (isCancelled ? 'Assinar' : 'Bloqueado') : 'Praticar'}
                  </button>
                </div>
              </div>
            ))}
